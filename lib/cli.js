 ...
 */
 ...
 cli.responders.exit = function(){
   console.log("Session has ended, The server is down, Goodbye!!!");
   process.exit(0);
 };
 ...
 cli.responders.stats = function(){
   ...
   var stats = {
     ...
     'Currnet Malloced Memory' : v8.getHeapStatistics().malloced_memory,
   ...
   cli.horizontalLine();
   cli.centered('SYSTEM STATISTICS RESULTS');
   cli.horizontalLine();
   cli.verticalSpace(2);
 ...
 cli.responders.listUsers = function(){
  ...
  cli.horizontalLine();
  cli.verticalSpace(2);
   _data.list('users', function(err, userId){
     if(!err && userId && userId.length > 0){
      cli.verticalSpace();
      userId.forEach(function(userId) {
 ...
 cli.responders.moreUserInfo = function(str){
   //Get the id form the string provided
   var arr = str.split('--');
     ...
     if(!err && checkIds && checkIds.length > 0){
       ...
       checkIds.forEach(function(checkId){
         _data.read('checks', checkId, function(err, checkData){
           ...
           //Getting the state of the check
           ...
           //When specified the state or not the state shown is the currnet state
